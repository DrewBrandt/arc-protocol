                                   ...
                                   uint8_t* out, size_t out_capacity)
{
    ...
    out[0] = (uint8_t)msg->tx_power_dbm;
...
{
    if (msg == NULL || out == NULL) return ARC_ERR_BAD_ARG;
    if (out_capacity < ARC_RADIO_SET_PHY_PROFILE_PAYLOAD_SIZE) return ARC_ERR_BUFFER;
    out[0] = msg->profile_id;
    return ARC_RADIO_SET_PHY_PROFILE_PAYLOAD_SIZE;
}
...
{
    if (in == NULL || msg == NULL) return ARC_ERR_BAD_ARG;
    if (len != ARC_RADIO_SET_PHY_PROFILE_PAYLOAD_SIZE) return ARC_ERR_BAD_LENGTH;
    msg->profile_id = in[0];
    return ARC_OK;
}
